...
class Simplify():
	...
	def removeNamespaceDependency(ast):
		...
		elif isinstance(ast,List): return List([Simplify.removeNamespaceDependency(node) for node in ast.nodes])
	...
	def nameToBool(ast):
		...
		elif isinstance(ast,List): return List([Simplify.nameToBool(node) for node in ast.nodes])
